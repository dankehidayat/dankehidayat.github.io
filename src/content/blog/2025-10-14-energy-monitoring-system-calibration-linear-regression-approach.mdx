...
Instead of a fancy model, I used ordinary linear regression. The idea is simple: map a raw DHT11 reading $x$ onto a calibrated value $\hat{y}$ that tracks the HTC-1.

$$
\hat{y} = ax + b
$$

Here $a$ is the slope and $b$ is the intercept, fit separately for temperature and humidity. After training I got roughly:

$$
\begin{aligned}
\hat{T} &= 1.343\,T_{\text{raw}} - 7.689 \\
\hat{H} &= 0.914\,H_{\text{raw}} + 5.127
\end{aligned}
$$

Mean absolute error after calibration is just the average gap against the reference:

$$
\mathrm{MAE} = \frac{1}{n}\sum_{i=1}^{n}\left|\hat{y}_i - y_i\right|
$$

Light enough for embedded hardware, still useful outdoors in a real room.
